...
ssh $TARAGET_HOST "cd ${UPLOAD_BIN_PATH}; ls -l |grep EchoTRM| grep 3.5>version.txt"
scp $TARAGET_HOST:$UPLOAD_BIN_PATH/version.txt $JENKINS_PATH/version.txt
ssh $TARAGET_HOST "rm ${UPLOAD_BIN_PATH}/version.txt"
...
echo "Checking version in ${SOMETHING} (Thai Jenkins)"
...
scp $TARAGET_HOST:$UPLOAD_BIN_PATH/*$newVersion*.tar.gz $SOMETHING/$newVersion
