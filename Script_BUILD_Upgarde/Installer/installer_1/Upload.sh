...
echo "Welcome to Upload.sh"
	...
	ssh $HOST "cd ${UPLOAD_BIN_PATH}; ls -l |grep EchoTRM| grep 3.5>version.txt"
	scp $HOST:$UPLOAD_BIN_PATH/version.txt $JENKINS_PATH/version.txt
	ssh $HOST "rm ${UPLOAD_BIN_PATH}/version.txt"
	...
	scp $HOST:$UPLOAD_BIN_PATH/*$newVersion*.tar.gz .
